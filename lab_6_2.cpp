...
// Таблица S-box для SubWord (256 элементов) замена байт
...
// Константы Rcon для ключевого расширения (11 значений) (константы для раундовых ключей)
...
void AES_EncryptBlock(const std::vector<uint8_t> &input, std::vector<uint8_t> &output, const std::vector<uint32_t> &roundKeys) {
    ...
    // Копируем вход в state (4x4 буем 44 слова (4 байта каждое) ГЕНЕРАЦИЯ РАУНДОВЫХ КЛЮЧЕЙ
    uint8_t state[4][4];
    for (int i = 0; i < 16; ++i) {
